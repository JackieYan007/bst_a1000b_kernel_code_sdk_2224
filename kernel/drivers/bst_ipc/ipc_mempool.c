	...
	struct ipc_memblock** memblock)
{
	...
	block = devm_kzalloc(&g_mbox_controller_platform_dev->dev, sizeof(struct ipc_memblock), GFP_KERNEL);
	if (!block)
		return -ENOMEM;
	...
	k_addr = dma_alloc_coherent(mempool->dev,
						...
						GFP_KERNEL);
	if (!k_addr) {
		devm_kfree(&g_mbox_controller_platform_dev->dev,  block);
		return -ENOMEM;
	}
...
static void ipc_cma_free(struct ipc_memblock *memblock)
{
	...
	mutex_unlock(&memblock_mutex);
	devm_kfree(&g_mbox_controller_platform_dev->dev, memblock);
}
...
{
	if(pool)
		devm_kfree(&g_mbox_controller_platform_dev->dev, pool);
}
...
int32_t bst_alloc(struct ipc_buffer * ipc_buffer, bool from_user)
{
	...
	struct bstipc* bstipc = platform_get_drvdata(g_mbox_controller_platform_dev);
...
int32_t bst_free(phys_addr_t phy_addr, bool from_user)
{
	...
	bstipc = platform_get_drvdata(g_mbox_controller_platform_dev);
	if (!bstipc) {
