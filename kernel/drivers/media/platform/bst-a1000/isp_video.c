 ...
 * Helper functions
 */
static int handle_video_sync_msg(struct a1000_isp_video *video,
...
static void reconfig_isp_ipc_tx_timeout(struct a1000_isp_device *isp)
{
	u32 media_cmd_paddr;
	struct media_command *media_cmd;
	isp_ipc_cfg_t *ipc_conf_cmd;
	ipc_msg msg;
	int ret;
	int camera_num;
	...
	media_cmd->cmd_hdr.hdr_info.cmd_type_minor = MINOR_ISP_IPC_CFG;
	ipc_conf_cmd = (isp_ipc_cfg_t *)(&media_cmd->user_cmd_data[0]);
	camera_num = atomic_read(&(isp->streamon_count));
	pr_info("Stream on cameras: %d\n", camera_num);
	switch (camera_num) {
	case 1:
		ipc_conf_cmd->txTimeOut = 32000; /* 32000*0.5us=16ms */
		break;
	case 2:
		ipc_conf_cmd->txTimeOut = 16000; /* 16000*0.5us=8ms */
		break;
	case 3:
		ipc_conf_cmd->txTimeOut = 10000; /* 10000*0.5us=5ms */
		break;
	case 4:
		ipc_conf_cmd->txTimeOut = 8000; /* 8000*0.5us=4ms */
		break;
	case 5:
	case 6:
		ipc_conf_cmd->txTimeOut = 6000; /* 6000*0.5us=3ms */
		break;
	case 7:
	case 8:
		ipc_conf_cmd->txTimeOut = 4800; /* 4800*0.5us=2.4ms */
		break;
	case 9:
	case 10:
		ipc_conf_cmd->txTimeOut = 3600; /* 3600*0.5us=1.8ms */
		break;
	case 11:
	case 12:
	default:
		ipc_conf_cmd->txTimeOut = 2400; /* 2400*0.5us=1.2ms */
		break;
	}
	...
	if (ret)
		pr_err("Failed to reconfig IPC txTimeout: %d\n", ret);
}
				     ...
				     unsigned int count)
{
	...
	if (video->channel->cam_dev->raw_cap_enabled) {
		/* When raw data need to be captured, ISP firmware can't send
		 * consitent yuv and raw buffers at the same time, this will
		 * lead to V4L2's qbuf and dqbuf operations fail, so we use
		 * firmware's reserved buffer to skew certain offset, then yuv
		 * and raw buffer will keep insistent, if we don't do these,
		 * this case send buf0: A B C send buf1: D E F will get recv 0 0
		 * C recv A B F
		 */
		pr_info("%s: video_index: %02d, cap raw is enabled, cap_buf_shift: %d\n",
			__func__, video->video_index,
			video->channel->cam_dev->cap_buf_shift);
		for (i = 0; i < video->channel->cam_dev->cap_buf_shift; i++) {
			uint32_t buf_paddr[3];

			buf_paddr[0] = 0;
			buf_paddr[1] = 0;
			buf_paddr[2] =
				video->isp->fbuf_paddr +
				(video->video_index * 3 +
				 i) * video->channel->views_video.cap_raw_size;
			pr_info("%s: video_index: %02d, buf_paddr[2]: 0x%08X\n",
				__func__, video->video_index, buf_paddr[2]);
			send_buf_to_fw(video, buf_paddr, 0);
		}
	}
	...
	for (i = 0; i < MIN_FRAME_BUFFER_NUM; i++) {
		...
		if (!list_empty(&video->free_buf_queue)) {
			...
			mutex_unlock(&video->free_queue_lock);
			buf->cycle_count = 0;
			send_buf_to_fw(video, buf->dma, buf->vb.vb2_buf.index);
	...
	*pview_id = *pcurrent_id;
	...
	reconfig_isp_ipc_tx_timeout(video->isp);
	return ret;
}
...
static void isp_video_stop_streaming(struct vb2_queue *queue)
{
	...
	dev_info(video->isp->dev, "=====%s\n", __func__);

	// mutex_lock(&video->irqlock);
	...
	if (ret < 0)
		pr_err("notify_fw_stop_stream error, ret = %d\n", ret);
				       ...
				       struct v4l2_format *format)
{
	...
	if (index == 2 && video->channel->cam_dev->raw_cap_enabled) {
		pr_info("%s: capture mode uses raw size: %u from firmware\n",
			__func__, video->cap_raw_size);
		vfh->format.fmt.pix_mp.plane_fmt[index].sizeimage =
			video->cap_raw_size;
	} else {
				     ...
				     struct resolution_resize *resize_t)
{
	...
	/*Crop After*/
	if (crop_size.botCropAfter - crop_size.topCropAfter > 0) {
		if (crop_size.lefCropAfter == 0) {
			video->views[index].width = crop_size.rigCropAfter -
						    crop_size.lefCropAfter + 1;
		} else {
			video->views[index].width =
				crop_size.rigCropAfter - crop_size.lefCropAfter;
		}
		if (crop_size.topCropAfter == 0) {
			video->views[index].height = crop_size.botCropAfter -
						     crop_size.topCropAfter + 1;
		} else {
			video->views[index].height =
				crop_size.botCropAfter - crop_size.topCropAfter;
		}
		pr_info("resize view[%d] to w:%d, h:%d !\n", index,
			video->views[index].width, video->views[index].height);
	} else if (scale_size.width > 0 && scale_size.height > 0) {
		video->views[index].width = scale_size.width;
		video->views[index].height = scale_size.height;
		pr_info("resize view[%d] to w:%d, h:%d !\n", index,
			video->views[index].width, video->views[index].height);
	} else {
		pr_err("invalid resize !\n");
	}
}
...
static int isp_video_open(struct file *file)
{
	...
	if (!video->enabled) {
		dev_info(video->isp->dev, "%s video %d disabled\n", __func__,
			 video->video_index);
		return -ENOENT;
	}
	mutex_lock(&(video->mutex));
...
{
	return video->is_raw ? isp_set_awb_raw(video, val) :
		       ...
		       enum v4l2_exposure_auto_type auto_exposure)
{
	if (auto_exposure == V4L2_EXPOSURE_AUTO) {
		video->is_raw ? isp_set_aec_raw(video, 1) :
				isp_set_aec_yuv(video, 1);
	} else {
		video->is_raw ? isp_set_aec_raw(video, 0) :
...
{
	return video->is_raw ? isp_set_gamma_raw(video, val) :
...
static int isp_set_manual_wb(struct a1000_isp_video *video,
			     struct v4l2_ctrl *ctrl)
{
	return video->is_raw ? isp_set_manual_wb_raw(video, ctrl) :
...
static int isp_set_manual_aec(struct a1000_isp_video *video,
			      struct v4l2_ctrl *ctrl)
{
	return video->is_raw ? isp_set_manual_aec_raw(video, ctrl) :
					   ...
					   struct camera_dev *cam_dev)
{
	...
	if (view_fmt != View2_Dis) {
		...
		if (cam_dev->raw_cap_enabled) {
			pr_info("%s: capture mode, channel %u use raw id: %u, s00: %u\n",
				__func__, video->chn_index,
				(video->chn_index + S00RawId), S00RawId);
			video->views[2].cmd_view_id =
				(video->chn_index + S00RawId);
		} else {
			video->views[2].cmd_view_id =
				((video->chn_index * VIEW_ID_NUM_PER_CAMERA) +
				 S00View2);
...
static int update_pdns_from_camera_config(struct a1000_isp_video *pdns_video,
					  struct camera_dev *cam_dev)
{
	if (cam_dev->isp_data.pdnsinfo.pdnsMode) {
		int inputview = cam_dev->isp_data.pdnsinfo.pdnsViewSel;
		int view_fmt = cam_dev->isp_data.viewinfo[inputview].viewFmt;

		pdns_video->enabled = true;
		pdns_video->views[0].enable = true;
		pdns_video->views[0].index = 0;
		pdns_video->views[0].video = pdns_video;
		pdns_video->views[0].width =
			cam_dev->isp_data.viewinfo[inputview].width;
		pdns_video->views[0].height =
			cam_dev->isp_data.viewinfo[inputview].height;
		pdns_video->views[0].format =
			convert_view_format_to_v4l2(inputview, view_fmt);
		pdns_video->views[0].cmd_view_id =
			((pdns_video->chn_index * VIEW_ID_NUM_PER_CAMERA) +
			 S00ViewPdns);
	}
				 ...
				 int chn_sn)
{
	...
	if (cam != NULL) {
		...
		video->is_raw = ((type != DT_UYVY) && (type != DT_YUYV));
...
int isp_channel_init_pdns_video(struct a1000_isp_device *isp,
				struct bst_isp_channel *isp_channel, int chn_sn)
{
	...
	video->isp = isp;
	snprintf(video->video.name, sizeof(video->video.name),
		 "isp-channel-%d-pdns-video", chn_sn);

	video_sn = chn_sn + MAX_ISP_CHANNEL; // 12 ~ 23
	video->enabled = false; // will be updated from config
	video->chn_index = chn_sn;
	video->video_index = video_sn;
	video->channel = isp_channel;
	...
	if (cam != NULL) {
		...
		video->is_raw = ((type != DT_UYVY) && (type != DT_YUYV));
		update_pdns_from_camera_config(video, cam);
	}
	a1000_isp_video_init(isp, video, video_sn);
	video->pad.flags = MEDIA_PAD_FL_SINK;
	ret = media_entity_pads_init(&video->video.entity, 1, &video->pad);
