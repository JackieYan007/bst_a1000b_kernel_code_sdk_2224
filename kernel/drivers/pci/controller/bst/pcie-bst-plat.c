...
static void a1000_pcie_reset_bus(struct device *dev)
{
	int ret;
	u32 gpio;
				 ...
				 struct platform_device *pdev)
{
	...
	if (IS_ENABLED(CONFIG_PCI_MSI)) {
		...
		pr_info("---, pp->msi_irq:%d", pp->msi_irq);
		if (pp->msi_irq < 0)
			return pp->msi_irq;
	}

	pp->ops = &dw_plat_pcie_host_ops;
	pci->dma_base = pci->dbi_base + 0x380000;
	pr_info("pci->dbi_base:%#lx dma:%#lx\n", (uintptr_t) pci->dbi_base, (uintptr_t) pci->dma_base);

	a1000_pcie_reset_bus(dev);
	...
	//pci_bus_read_config_dword(bus, 0, PCI_COMMAND, &reg_val);
	dw_pcie_read(pp->va_cfg0_base + PCI_COMMAND, 4, &reg_val);
	//pr_err("** %s, %d reg_val:%#x\n", __func__, __LINE__, reg_val);
	reg_val &= 0xfffffff8;
	reg_val |= 0x7;
	//pci_bus_write_config_dword(bus, 0, PCI_COMMAND, reg_val);
	dw_pcie_write(pp->va_cfg0_base + PCI_COMMAND, 4, reg_val);
			       ...
			       struct platform_device *pdev)
{
	...
	if (!res) {
		pr_err("fail platform_get_resource_byname dma\n");
		return -EINVAL;
	}

	pci->dma_base = devm_ioremap_resource(dev, res);
...
static int pcie_ltssm_detect(void *kkk)
{
	...
	offset = dw_pcie_find_capability(pci, PCI_CAP_ID_EXP);
	pci_root_dev = pci_get_device(0x16C3, 0xABCD, NULL);
	if (!pci_root_dev) {
		pr_info("Error retrieving PCI device structure\n");
		return false;
	}
	pcie_ltssm_enable(pci);
...
static void bst_pcie_dscan(struct dw_pcie *pci)
{
